"""
M4 cycle 25 — cross-anchor stabilizer follow-up to cycle 24.

Question.
  Cycle 24 produced two cross-anchor data points for the long-CEM ladder:
    * c21-s1 (median piecewise basin, lift_FF +2.900): n=3 mean lift over
      same-anchor piecewise = +0.134 ± 0.066.
    * c21-s2 (basin-collapsed piecewise, lift_FF +2.275): n=1 lift over
      same-anchor piecewise = +0.547 (single seed, supporting evidence
      only).

  Two open questions for cycle 25:

  Q1 (c21-s2 second seed). Is the cycle-24 +0.547 single-seed point
      stable, or is it a positive-tail draw of a wide distribution? A
      second RNG seed (rng_seed=1) on the c21-s2 anchor should land in
      [+0.30, +0.70] (consistent with first-seed +0.547 plus cycle-23-
      grade dispersion ±0.07-0.30) → n=2 mean above +0.40 confirms
      stabilizer.
  Q2 (c18-s0 anchor). The cycle-18-seed-0 piecewise anchor (lift_FF
      +3.006) is the strongest piecewise basin in the family. If the
      "anchor-conditional lift" hypothesis holds (long-CEM ladder lift
      over piecewise scales inversely with piecewise quality), c18-s0
      should give a SMALLER lift than c21-s1 (median, +0.13). Single
      seed (rng_seed=0) probe.

  Decision rules (combined cycle-24+25 picture):
    * If c21-s2 n=2 mean lift > +0.40 AND c18-s0 single-seed lift < +0.13
      (preferably < +0.05): inverse-scaling line confirmed at 3 anchors.
      Headline framing: "ladder is a stabilizer, not a frontier-mover."
    * If c21-s2 second seed ≈ 0: cycle-24 +0.547 was a positive tail
      draw; need more seeds to characterize the distribution.
    * If c18-s0 lift > c21-s1 mean (+0.13): inverse-scaling line is
      false; the family-vs-anchor relationship is not monotone.
...
Method.
  ...
  Two runs in parallel:
    A. ANCHOR_KEY=cycle21_seed2 RNG_SEED=1  (Q1; tightens c21-s2 to n=2)
    B. ANCHOR_KEY=cycle18_seed0 RNG_SEED=0  (Q2; new anchor probe)
...
def main() -> None:
    anchor_key = os.environ.get("ANCHOR_KEY", "cycle21_seed2")
    ...
    outdir = (
        ROOT
        / f"research/experiments/2026-05-06-cycle25-m4-cross-anchor-stabilizer/results/{anchor_key}_seed{rng_seed}"
    )
    ...
    _log(
        f"M4 cycle-25 ladder LONG-CEM | anchor={anchor_key} rng_seed={rng_seed} "
