"""
Cycle-8 M2 — inventory-aware piecewise warm-start CEM.

Anchor = cycle-6 piecewise best (16 params, val 433.57 / test 432.75) lifted
into the 19-param `inventory_aware_piecewise` family by setting the 3 new
inventory-skew parameters to zero. By construction the score at this
anchor equals the cycle-6 piecewise score, so any positive delta is
attributable to the inventory dimension.

Driver intentionally mirrors cycle-7's warm-start CEM driver
(`research/experiments/2026-05-04-cycle7-multi-family-warmstart-cem/scripts/run_warmstart_cem.py`)
so the cycle-7 hyperparameter ablation (pop=24, gen=12, init_std_frac=0.10,
elite_frac=0.20, search_seeds 0..63, val 1000..1127, test 2000..2255) carries
over without re-tuning.

Outputs:
  research/experiments/2026-05-04-cycle8-inventory-piecewise/results/
      inventory_warmstart_cem_history.json
      inventory_warmstart_cem_test.json
      inventory_warmstart_cem_progress.log
"""
...
from arena_search.simple_amm_search import (  # noqa: E402
    INVENTORY_AWARE_PIECEWISE_PARAM_RANGES,
    POLICY_SPECS,
)
...
# Cycle-6 piecewise best (val 433.57 on 128 val seeds, test 432.75 on 256
# test seeds). Loaded directly from cycle-6 result file at runtime.
CYCLE6_TEST_JSON = (
    ROOT
    / "research/experiments/2026-05-04-cycle6-m2-warmstart-cem/results/warmstart_cem_test.json"
)

# Cycle-6 piecewise best test score (the inventory-skew=0 anchor matches this
# by construction, modulo float noise in `normalized()`).
...
def _inherited_anchor() -> dict[str, float]:
    """Cycle-6 piecewise best plus zero inventory-skew tail."""
    with CYCLE6_TEST_JSON.open() as f:
        d = json.load(f)
    pw_params = dict(d["best_by_val"]["params"])
    pw_params["inventory_skew_to_bid"] = 0.0
    pw_params["inventory_skew_to_ask"] = 0.0
    pw_params["inventory_skew_dead_zone"] = 0.0
    return pw_params
...
def _logger():
    logfile = OUTDIR / "inventory_warmstart_cem_progress.log"
...
def main(rng_seed: int, generations: int) -> None:
    ...
    ranges = INVENTORY_AWARE_PIECEWISE_PARAM_RANGES
    ...
    log(
        f"Cycle-8 inventory-aware piecewise warm-start CEM | dim={len(names)} "
        f"pop={POPULATION} gen={generations} elite_n={elite_n} "
        f"init_std_frac={INIT_STD_FRAC} workers={MAX_WORKERS} rng_seed={rng_seed}"
    )
    log(f"Anchor (inv-skew zeros at end): {dict(zip(names, init_mean.tolist()))}")
    ...
    # Anchor sanity check: re-score the inherited piecewise best (zeros for
    # the inv-skew tail) on the search seeds and confirm it's near 414
    # (matching cycle-6 piecewise on 64 search seeds, not the 256-test 432).
    ...
    for gen in range(generations):
        ...
        # Track inv-skew elite means as a side signal — non-zero means CEM
        # is finding signal in the new dimensions.
        inv_skew_idx = [names.index(n) for n in (
            "inventory_skew_to_bid",
            "inventory_skew_to_ask",
            "inventory_skew_dead_zone",
        )]
        inv_skew_means = [float(new_mean[i]) for i in inv_skew_idx]
        ...
        gen_record = {
            ...
            "inv_skew_to_bid_mean": inv_skew_means[0],
            "inv_skew_to_ask_mean": inv_skew_means[1],
            "inv_skew_dead_zone_mean": inv_skew_means[2],
        ...
        log(
            ...
            f"inv_bid={inv_skew_means[0]:+.4f} inv_ask={inv_skew_means[1]:+.4f} "
            f"inv_dz={inv_skew_means[2]:.4f} ({gen_elapsed:5.1f}s+{val_elapsed:4.1f}s)"
        )

        (OUTDIR / "inventory_warmstart_cem_history.json").write_text(
            json.dumps(
                {
    ...
    log(
        ...
        f"piecewise_starting_line={INHERITED_BASELINE_TEST_SCORE:.3f}"
    )
    log(f"  Δ vs cycle-6 piecewise best: {delta:+.3f}")
    ...
    log(f"  -> wrote {OUTDIR / 'inventory_warmstart_cem_test.json'}")
