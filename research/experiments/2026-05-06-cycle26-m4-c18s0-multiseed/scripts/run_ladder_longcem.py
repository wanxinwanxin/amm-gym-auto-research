"""
M4 cycle 26 — multi-seed long-CEM ladder on the c18-s0 anchor.

Question.
  Cycle 25 introduced the c18-s0 piecewise anchor (lift_FF +3.006, the
  strongest piecewise basin in the family). A single-seed long-CEM
  ladder probe at rng_seed=0 lifted +0.250 over the same-anchor
  piecewise (test +3.727, lift_FF +3.256, retail_adv +3.541), which is
  *higher* than the c21-s1 n=3 mean (+0.134), rejecting the cycle-24
  inverse-scaling line and producing the highest single-seed
  real_data score recorded to date (n=1).

  Cycle 26 tightens c18-s0 from n=1 to n=3 with two more rng_seeds.
  The aggregated n=6 long-CEM ladder distribution from cycle 25 has
  mean lift over same-anchor piecewise ≈ +0.20 with sample stddev
  ±0.19 (bimodal: 5/6 in the "find" regime, 1/6 in the "collapse"
  regime). Drawing two more seeds from this distribution on c18-s0
  should land the n=3 c18-s0 mean somewhere in [+0.05, +0.40] with
  high probability.

  Decision rules:
    * If c18-s0 n=3 mean > +0.15 AND at least one new seed lands
      in [+0.10, +0.40]: c18-s0 is stronger than c21-s1 — the M4
      headline anchor for ladder-family multi-seed shifts to
      c18-s0 long-CEM ladder.
    * If at least one new seed collapses (margin ≤ −0.50): the
      bimodal mixture-of-regimes characterization extends from
      c21-s2 to c18-s0 — i.e. long-CEM has an anchor-independent
      collapse rate, not just anchor-specific.
    * If both new seeds collapse OR both come in below +0.05: the
      c18-s0 +0.250 single-seed lift was noise; c18-s0 anchor is
      not the M4 frontier; the long-CEM ladder ceiling on real_data
      sits near c21-s1's +3.034 ± 0.066 mean.

Method.
  Identical CEM mechanics to cycle 23/24/25 (19d ladder,
  ...
  val, 256 test, 6-deep rerank on val) and identical budget
  (POPULATION=24, GENERATIONS=10). Identity warm-start from c18-s0
  piecewise long-CEM best params (cycle 18) — same anchor params
  cycle 25 used, just at rng_seed ∈ {1, 2} instead of {0}.
...
Configuration (env vars):
  ANCHOR_KEY in {"cycle21_seed1", "cycle21_seed2", "cycle18_seed0"}
...
# Piecewise anchors. Lift_FF is the anchor's nominal real_data score
# minus FF baseline (0.4703 on the same n=256 test split).
#
#   cycle21_seed1: cycle 21 best-by-val piecewise long-CEM seed=1
#                  (test +3.370, lift_FF +2.900, retail_adv +3.820)
#   cycle21_seed2: cycle 21 best-by-val piecewise long-CEM seed=2
#                  (test +2.745, lift_FF +2.275, basin-collapsed)
#   cycle18_seed0: cycle 18 best-by-val piecewise long-CEM seed=0
#                  (test +3.476, lift_FF +3.006, strongest piecewise)
...
# Match cycle-19/22/23/24 dim-init settings; only POPULATION/GENERATIONS scale.
...
def main() -> None:
    ...
    rng_seed = int(os.environ.get("RNG_SEED", "1"))
    ...
    outdir = (
        ROOT
        / f"research/experiments/2026-05-06-cycle26-m4-c18s0-multiseed/results/{anchor_key}_seed{rng_seed}"
    )
    ...
    _log(
        f"M4 cycle-26 ladder LONG-CEM | anchor={anchor_key} rng_seed={rng_seed} "
