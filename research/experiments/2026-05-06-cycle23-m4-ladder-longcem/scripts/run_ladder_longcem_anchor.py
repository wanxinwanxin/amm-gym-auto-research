"""
M4 cycle 23 — long-CEM ladder warm-started from a piecewise anchor.

Question.
  Cycle 22 showed that the cycle-19 +0.245 ladder lift over piecewise was
  anchor-driven, not a representational gain — at the *short-CEM* budget
  (5 generations × 12 population). Cycle-22 also surfaced a clean
  hypothesis: maybe the ladder family payoff only manifests at the
  *long-CEM* budget (10 generations × 24 population, matching cycle-18's
  long-piecewise compute), because at that budget the search→val
  generalization gap closes.

  Cycle 23 tests that directly. We re-run the ladder CEM with cycle-18's
  long-CEM budget, warm-started from the cycle-21-seed-1 piecewise
  (median of the long-CEM piecewise distribution, test +3.370 / lift_FF
  +2.900). Decision rule:
    * lift_FF > +3.10 on test (>+0.20 over the seed-1 piecewise anchor):
      ladder family is real at long-CEM budget; pivot M4 to multi-seed
      long-ladder cross-anchor next cycle.
    * lift_FF in [+2.85, +3.10]: ladder is at-best a wash on this
      anchor at this budget; pivot to a structurally different family
      next cycle (e.g. 6-bucket ladder, smooth-head MLP).
    * lift_FF < +2.85: ladder is regressing; long-CEM is finding worse
      candidates than the anchor — interesting failure to investigate.

Method.
  Identical CEM mechanics to cycle 22 (19d, init_std_inh=0.05,
  init_std_new=0.15, elite_frac=0.2, normalizer FixedFee(0.003,0.003),
  evaluator real_data, 64 search seeds, 128 val, 256 test) except:
    POPULATION = 24  (was 12)
    GENERATIONS = 10 (was 5)
  These match cycle-18's long-piecewise budget on the same evaluator.

Configuration (env vars):
  ANCHOR_KEY = "cycle21_seed1" (only valid value for this experiment;
                                 the anchor confounding rule says we
                                 should not stack cross-anchor with
                                 cross-budget on the first run)
...
# Match cycle-19/22 dim-init settings; only POPULATION/GENERATIONS scale.
...
def main() -> None:
    ...
    outdir = (
        ROOT
        / f"research/experiments/2026-05-06-cycle23-m4-ladder-longcem/results/{anchor_key}_seed{rng_seed}"
    )
    ...
    _log(
        f"M4 cycle-23 ladder LONG-CEM | anchor={anchor_key} rng_seed={rng_seed} "
