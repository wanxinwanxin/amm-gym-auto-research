"""Per-trade-size retail decomposition of the cycle-18 long-budget c11+CEM
best-by-val params, mirroring cycle-17 eval_size_decomp_c11_mirror.py.

Question. Does the longer-budget CEM keep retail-positive in the small
bucket (c11+CEM cycle-17 was +1.45 small-bucket retail_adv on val), or
does the extra compute shift the optimizer toward the c5+CEM
arb-side basin?
"""
...
OUTDIR = (
    ROOT
    / "research/experiments/2026-05-05-cycle18-m4-prior-sweep/results"
)
ANCHOR_PATH = OUTDIR / "long_cem_from_c11/test.json"
...
def main() -> None:
    rng = np.random.default_rng(0xC18A)
    anchor_doc = json.loads(ANCHOR_PATH.read_text())
    params = anchor_doc["best_by_val"]["params"]
    print(f"Cycle 18 — long_cem_from_c11 best-by-val per-trade-size decomp; n_seeds={len(VAL_SEEDS)}")
    print(f"  test_lift_FF={anchor_doc['best_by_val']['lift_FF']:+.3f}  "
          f"test_score={anchor_doc['best_by_val']['test_score']:+.3f}")
    ...
    out_path = OUTDIR / "size_decomp_long_cem_from_c11.json"
