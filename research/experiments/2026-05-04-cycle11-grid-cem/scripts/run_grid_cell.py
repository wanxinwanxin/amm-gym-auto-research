"""
Cycle-11 grid CEM cell — generic warm-start CEM with optional inert no-op tail.

This script runs ONE cell of the cycle-11 (dim, rng_seed) grid. The grid
is defined externally (`run_grid.py` / nohup); each cell is a self-
contained CEM run that writes its own JSON under
`research/experiments/2026-05-04-cycle11-grid-cem/results/cell_d{DIM}_s{SEED}/`.

Why a grid: cycle-10 showed cycle-9's +7.83 lift was largely an artifact
of which RNG sequence the warm-start CEM consumed, not a property of
the wrapper geometry or the wrapper's signal. Cycle-10 sampled only 4
(dim, seed) cells out of a wide grid: (16,0), (16,1), (20,0), (20,0
EMA). This cycle adds three new cells:

  - (16-d, seed=2): a third 16-d seed to bound the seed-lottery std.
  - (24-d-noop, seed=0): does adding more inert tail dims at seed=0
    keep helping (because of more rng-stream offset)?
  - (24-d-noop, seed=1): the same dim at a different seed.

The 16-d core anchor is the cycle-8 ablation best (the same anchor
cycle-9 / cycle-10 used). Recipe parity (pop=24, gen=12, init_std_frac=
0.10, elite_frac=0.20) keeps this comparable to all prior cycle-N
warm-start CEMs.

Usage:
    python run_grid_cell.py --dim-total 16 --rng-seed 2
    python run_grid_cell.py --dim-total 24 --rng-seed 0
    python run_grid_cell.py --dim-total 24 --rng-seed 1

Each run takes ~30 min on this 4-core sandbox.
"""
...
def main(dim_total: int, rng_seed: int, generations: int) -> None:
    if dim_total < PIECEWISE_DIM:
        raise SystemExit(f"dim_total must be ≥ {PIECEWISE_DIM} (piecewise core).")
    num_noop_tail = dim_total - PIECEWISE_DIM

    cell_label = f"d{dim_total}_s{rng_seed}"
    outdir = (
        ROOT
        / f"research/experiments/2026-05-04-cycle11-grid-cem/results/cell_{cell_label}"
    )
    ...
    log(
        f"Cycle-11 grid cell {cell_label} | "
    ...
    if num_noop_tail > 0:
        log(f"Anchor (noop tail of {num_noop_tail} inert dims): all zeros")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dim-total", type=int, required=True)
    parser.add_argument("--rng-seed", type=int, required=True)
    parser.add_argument("--generations", type=int, default=GENERATIONS)
    args = parser.parse_args()
    main(args.dim_total, args.rng_seed, args.generations)
