"""
M4 cycle-3 — ladder-family CEM warm-started from c11+CEM long.

Question.
  The piecewise family (16 params, 3 size buckets) capped at +3.0 lift_FF
  on real_data after a long-budget CEM. Cycle-19 ablation localized the
  edge to base_fee (Δ-1.94), reversal_small (Δ-0.755), and to a lesser
  extent base_spread / continuation_small / large_trade_threshold. The
  small bucket is load-bearing: small trades carry +3.31 retail edge.

  Hypothesis: a 4-bucket ladder family (split small into tiny + small)
  warm-started from the c11+CEM long params lifts test lift_FF above the
  piecewise ceiling, by giving CEM finer resolution in the small-trade
  region where the optimizer's signal lives.

  Decision rule:
    > +3.50 lift_FF → richer family pays off; the piecewise ceiling
      was a representational limit, not a CEM compute limit.
    [+3.20, +3.50] → marginal; the extra dim helps but doesn't
      structurally change the picture.
    < +3.20 → escalation doesn't help; the small-bucket pricing
      surface is already saturated under the piecewise representation.

Method.
  Identical CEM mechanics to cycle-18 long (except dim=19, pop=12,
  gen=5, init_std_frac smaller for inherited dims):
    - normalizer: FixedFee(0.003, 0.003)
    - evaluator: real_data
    - search seeds: range(0, 64); val seeds: range(1000, 1128);
      test seeds: range(2000, 2256)
    - rerank: anchor + top 6 unique elites by search score, picked
      best by val score
    - init_std_frac: 0.05 for the 16 inherited dims, 0.10 for the
      3 new dims (tiny_threshold, continuation_tiny, reversal_tiny)
    - elite_frac: 0.2

  Warm-start. tiny_trade_threshold = small_trade_threshold * 0.5;
  continuation_tiny = continuation_small; reversal_tiny = reversal_small.
  This yields exactly the c11+CEM long behaviour on the empirical size
  distribution at gen 0 (verified 0.000 score diff on 8 challenge
  seeds), giving CEM a no-loss starting point with extra dims to
  differentiate.
"""
...
OUTDIR = (
    ROOT
    / "research/experiments/2026-05-05-cycle19-m4-ablation-and-ladder/results/ladder_cem"
)
...
# Wider std for the 3 new dims (give CEM room to differentiate the new
# bucket); tighter std for the 16 inherited dims (the c11+CEM long region
...
def main() -> None:
    ...
    _log(
        f"M4 cycle-3 ladder-family CEM | dim={len(names)} | "
    ...
    # Anchor (extended c11_long). MUST score very close to c11_long itself.
    ...
    for gen in range(GENERATIONS):
        ...
        # Order by candidate_id index for reproducibility
    ...
    # Inject anchor first to guarantee best_by_val cannot regress past it
    ...
    # FF baseline on the same test seeds for lift_FF
