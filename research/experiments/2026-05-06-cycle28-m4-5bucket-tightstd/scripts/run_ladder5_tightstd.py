"""
M4 cycle 28 — 5-bucket ladder long-CEM with TIGHTENED init_std on
new dims (init_std_frac_new=0.05 instead of cycle-27's 0.15).

Question.
  Cycle 27 ran a 22-d 5-bucket ladder on the c18-s0 anchor at the
  cycle-26 budget (10g × 24p, init_std_frac_new=0.15) and produced
  cluster mean lift_FF +3.093 ± 0.123 (n=2), −0.184 below the
  cycle-26 4-bucket cluster mean +3.277 ± 0.027. Mechanistic
  diagnosis from cycle 27: the 22-d landscape has 2× the new-dim
  init noise energy of the 19-d 4-bucket, the 5-bucket elite-mean
  trajectory crosses the anchor 2–7 generations later than the
  4-bucket, and seed 1 reranks back to the anchor (CEM never lifts
  above warm-start val).
  ...
  Frontier threshold +3.28 (= cluster mean within rounding). For n=2:
    * Both seeds > +3.28 AND mean lift_FF >= +3.28 → tightened std
      lifts the 5-bucket cluster onto the 4-bucket frontier; the
      cycle-27 underperformance was variance-bounded; cycle 29
      tightens with seed=2 (n=3).
    * At least one seed < +3.06 (= piecewise level on c18-s0) →
      tightening hurt the family; the wider exploration at 0.15
      was actually doing useful work for the gen-9 elites that
      did find something. (Possible regime: with std=0.05 the new
      dims barely move and the rerank floor is the anchor.)
    * Cluster mean in [+3.10, +3.28] → tightening helps a little
      but does not close the gap; family is the binding constraint
      on c18-s0 at long-CEM. Cycle 29 should test (15g × 24p)
      compute lift independently to disambiguate fully.

Method.
  Identical to cycle-27 driver except init_std_frac_new=0.05
  (same as inherited dims). Same warm-start (direct from c18-s0
  piecewise), same CEM mechanics: pop=24, gen=10, elite_frac=0.2,
  normalizer FixedFee(0.003, 0.003), evaluator real_data, 64
  search seeds, 128 val, 256 test, 6-deep rerank-by-val.
...
def main() -> None:
    ...
    outdir = (
        ROOT
        / f"research/experiments/2026-05-06-cycle28-m4-5bucket-tightstd/results/{anchor_key}_seed{rng_seed}"
    )
    ...
    _log(
        f"M4 cycle-28 5-bucket-ladder TIGHTSTD LONG-CEM | anchor={anchor_key} rng_seed={rng_seed} "
