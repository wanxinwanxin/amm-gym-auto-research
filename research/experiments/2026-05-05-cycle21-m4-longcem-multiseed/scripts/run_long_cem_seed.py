"""
M4 cycle-21 — multi-seed long-CEM on c11+CEM long.

This is a parameterized re-run of cycle-18's run_long_cem_from_c11.py.
Parameters set via env vars:

  RNG_SEED       : integer rng seed for the CEM stream (default 0)
  RUN_TAG        : output subdir name under results/ (default seed{RNG_SEED})
  MAX_WORKERS    : override workers (default cpu_count-1)

Same warm-start (c11_d16_s2 best-by-val), same evaluator (real_data),
same search/val/test seed splits, same population/generations. The
*only* axis we are varying is the rng_seed that draws CEM populations.

Decision rule on cross-seed mean lift_FF (combined with cycle-18
seed=0 result of +3.006):

  - mean > +3.20 : +3.0 was a budget/anchor-search-noise ceiling, not
    a population ceiling. Family is neutral; use compute, not family.
  - mean in [+3.00, +3.20] : +3.0 is a population ceiling under
    this evaluator + warm-start + family combination. Family
    escalation is the next move (with >=3 seeds from start).
  - mean < +3.00 : cycle-18 single-seed +3.006 was on the high end;
    the ceiling is even tighter.
"""
...
RUN_TAG = os.environ.get("RUN_TAG", f"seed{RNG_SEED}")
...
MAX_WORKERS = int(os.environ.get("MAX_WORKERS", str(_DEFAULT_WORKERS)))
...
def main() -> None:
    ...
    _log(
        f"M4 cycle-21 c11 long-CEM | piecewise on {EVALUATOR_KIND} | rng_seed={RNG_SEED} "
        f"pop={POPULATION} gen={GENERATIONS} elite_n={elite_n} "
        f"init_std_frac={INIT_STD_FRAC} workers={MAX_WORKERS}"
    )
