"""
Cycle-10 Experiment B — 16-d 4th-pass piecewise CEM, rng_seed=1.

Companion control to `run_noop_tail_cem.py`. Tests hypothesis (B): the
+7.83 lift cycle-9 #3 saw over cycle-9 #1 was just rng_seed lottery
(different population samples drawn from the same warm-start anchor).

Identical recipe to cycle-9 #1 (third-pass piecewise CEM) except for
the RNG seed: pop=24, gen=12, init_std_frac=0.10, elite=0.20, anchor =
cycle-8 ablation best (16 dims). Different seed = different random
sampling sequence.

Reference: cycle-9 #1 (rng_seed=0) hit val 450.05, test 448.81. If
this run with rng_seed=1 hits ≈ 449 too, hypothesis (B) is weak. If it
hits ≈ 456, the wrapper-as-noise framing is wrong — it's just rng
variance.
"""
...
def _logger():
    logfile = OUTDIR / "seed_lottery_cem_progress.log"
...
def main(rng_seed: int, generations: int) -> None:
    ...
    log(
        f"Cycle-10B seed-lottery 16-d piecewise CEM | dim={len(names)} "
    ...
    log(f"Anchor search-seed score: {anchor_check['score']:.3f}")
    ...
    for gen in range(generations):
        ...
        (OUTDIR / "seed_lottery_cem_history.json").write_text(
            json.dumps(
                {
                    "policy_family": FAMILY,
                    "init_strategy": "warm_start_cycle8_ablation",
    ...
    for elite in sorted(all_elites, key=lambda r: r["score"], reverse=True):
        key = tuple(round(elite["params"][n], 12) for n in names)
        if key in seen:
            continue
        seen.add(key)
    ...
    (OUTDIR / "seed_lottery_cem_test.json").write_text(json.dumps(final, indent=2))
    log(f"  -> wrote {OUTDIR / 'seed_lottery_cem_test.json'}")
